...
/* Text Format: mmmmmmmmmm o AAAAAAAAAA NNNNNNNNNN ddddddddddn0 */
#define MMC_LOG_MAX_STR_LEN              47
...
int mmc_log_transaction(u8 mmc_operation, u32 block_addr, u16 num_blocks, u32 start_msecs, u16 duration_msecs)
{
    ...
    g_ps_log_buffer[g_write_index].time_ms     = start_msecs;
    g_ps_log_buffer[g_write_index].mmc_op      = (mmc_operation & 0x01);
    g_ps_log_buffer[g_write_index].block_num   = (block_addr & 0x7FFFFFFF);
    g_ps_log_buffer[g_write_index].num_blocks  = num_blocks;
    g_ps_log_buffer[g_write_index].duration_ms = duration_msecs;
...
static ssize_t mmc_log_read(struct file *p_file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    while ((count > 0) && (g_num_unread_entries > 0))
    {
        snprintf(szLogEntry,
                 ...
                 g_ps_log_buffer[g_read_index].duration_ms);
